...
trait DCE_Trait_Elementor {
    ...
    public static function get_post_id_by_element_data($data, $post_id = 0) {
        /*
          ...
          $db_data = wp_slash( $json_data );
          global $wpdb;
          $table = $wpdb->prefix . 'postmeta';
          //$query = "SELECT post_id FROM " . $table . " WHERE meta_value LIKE '%[{\"id\":\"".$element_id."\",%'";
          $query = "SELECT post_id FROM " . $table . " WHERE meta_key LIKE '_elementor_data' AND meta_value LIKE '" . $db_data . "'";
          var_dump($query);
        ...
        if (is_user_logged_in()) {
            ...
            if (is_singular()) {
                if (\Elementor\User::is_current_user_can_edit_post_type(get_post_type())) {
                    //return \Elementor\User::is_current_user_can_edit();
                    return true;
                }
            } else {
    ...
    public static function template_unwrap($html = '') {
        ...
        if($pos !== false) {
            ...
            $tmp = explode('<', $tmp);
            array_pop($tmp);array_pop($tmp);array_pop($tmp);
            $pre = implode('<', $tmp);
