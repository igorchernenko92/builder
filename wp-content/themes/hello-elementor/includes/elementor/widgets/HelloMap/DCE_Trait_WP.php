...
trait DCE_Trait_WP {
    ...
    public static function get_post_terms( $post_id = 0, $taxonomy = null, $args = array(), $targs = array('public' => true)) {
        ...
        if (!empty($post_taxonomies)) {
            foreach ($post_taxonomies as $key => $atax) {
                ...
                //echo '<h1>'.$atax.'</h1>'; var_dump($tmp_terms);
    ...
    public static function get_taxonomy_terms($taxonomy = null, $flat = false, $search = '', $info = true) {
        $listTerms = [];
        $flatTerms = [];
        $listTerms[''] = 'None';
...
// ************************************** ALL POST SINGLE IN ALL REGISTER TYPE ***************************/
    ...
    public static function get_post_value($post_id = null, $field = 'ID', $sub_field = '', $single = null) {
        ...
        if ($postValue === null) { // fot meta created with Toolset plugin
            if (metadata_exists('post', $post_id, 'wpcf-' . $field)) {
                $postValue = get_post_meta($post_id, 'wpcf-' . $field, $single);
            }
        }
        
        if ($postValue === null) { // fot meta WooCoomerce plugin
    ...
    public static function get_term_value($term = null, $field = 'name', $single = null) {
        ...
        if ($termValue === null) { // fot meta created with Toolset plugin
    ...
    public static function get_queried_object_type() {
        ...
        if (is_object($queried_object)) {
            switch(get_class($queried_object)) {
                case 'WP_Term': $queried_object_type = 'term'; break;
                case 'WP_User': $queried_object_type = 'user'; break;
            }
        }
    ...
    public static function get_permalink($obj = null, $fallback = false) {
        ...
        if (is_array($obj)) {
            if (isset($obj['term_id'])) {
                return self::get_term_link($obj['term_id']);
            }
        ...
        if (is_object($obj)) {
            ...
            if ($val_class == 'WP_Term') {
                return self::get_term_link($val->term_id);
            }
...
/*
// when Taxonomy Category is used on NON "post" Posts
// https://wordpress.stackexchange.com/questions/22528/custom-post-type-pagination-404-fix
add_action( 'parse_query', 'change_post_type' );
